...
class TestHelpers(unittest.TestCase):
    ...
    def test_has_ext_of(self):
        self.assertEqual(
            has_ext_of(Path("/alfa/bra.vo/charlie.ogg"), "OGG"), True)
        self.assertEqual(
            has_ext_of(Path("/alfa/bra.vo/charlie.ogg"), ".ogg"), True)
        self.assertEqual(
            has_ext_of(Path("/alfa/bra.vo/charlie.ogg"), "mp3"), False)
    ...
    def test_make_initials(self):
        ...
        self.assertEqual(
            make_initials("A. Strugatsky, B...Strugatsky."), "A.S.,B.S.")
        self.assertEqual(
            make_initials("Иржи Кропачек, Йозеф Новотный"), "И.К.,Й.Н.")
